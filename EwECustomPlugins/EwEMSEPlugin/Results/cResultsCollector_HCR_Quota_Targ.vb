        ...
        For igrp = 1 To m_MSE.Core.nGroups
            For iFleet = 1 To m_MSE.Core.nFleets
                For iTime = 1 To NumberOfTimeRecords
                    Me.SetValue(StrategyIndex, igrp, iFleet, iTime) = m_MSE.HCR_Quota_Target(igrp, iFleet, iTime)
                    If Me.GetValue(StrategyIndex, igrp, 0, iTime) = DefaultValue And m_MSE.HCR_Quota_Target(igrp, iFleet, iTime) <> DefaultValue Then
                        Me.SetValue(StrategyIndex, igrp, 0, iTime) = 0
                    End If
                    If Me.GetValue(StrategyIndex, igrp, 0, iTime) <> DefaultValue And m_MSE.HCR_Quota_Target(igrp, iFleet, iTime) <> DefaultValue Then
                        Me.SetValue(StrategyIndex, igrp, 0, iTime) = Me.GetValue(StrategyIndex, igrp, 0, iTime) + m_MSE.HCR_Quota_Target(igrp, iFleet, iTime) 'Summing across fleets
                    End If
                Next
            Next
        Next
